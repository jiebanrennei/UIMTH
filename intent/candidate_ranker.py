...
class Candidate_Ranker(object):
    ...
    def __init__(self,model_path = 'models/bert'):
        ...
        self.dialogues = []
        self.dialogues_path = '../data_tgcn/mr/build_train/dialogues1.txt'
        f = open(self.dialogues_path, 'r', encoding="utf-8")
        lines = f.readlines()
        for line in lines:
    ...
    def search_hard_negatives(self, anchor = 'dialogue', dialogue = None, true_candidate = None, sampling_method = 'topK', positive_threshold = 0.5, beta = 1,num_negatives = 1,index = 0):
        ...
        if anchor == 'dialogue':
           ...
           dialogues = []
           dialogues.append(true_candidate)
           # dialogues.append(dialogue)
           location1 = '.tcchinesefen.txt'
           location2 = '.tcdata.json'
           Syntactic1.Syntactic1(location1, location2, index)
           location = '/{}_dialogues.pkl'
           Semantic.Semantic(dialogues,location)
           location1 = '.dialogues_fen.txt'
           location2 = '/{}_dialogues.pkl'
           location3 = '_dialogues.pkl'
           build_graph.build_graph(location1,location2,location3)
           location1 = "../data_tgcn/mr/lstm/mr_dialoguesemb.pkl"
           location2 = "../data_tgcn/mr/lstm/mr_dialoguesemb.pkl"
           emb = trainC.trainC(location1,location2)
           candidate_embedding = emb[len(emb) - 1]
           # dialogues_embedding = emb[len(emb) - 1]
           a = candidate_embedding
           # b = dialogues_embedding
           dialogues_embeddings = emb[:len(emb) - 1, :]
           hits = util.semantic_search(candidate_embedding, dialogues_embeddings, top_k=len(self.dialogues))
           hits = hits[0]



           hits = [(self.dialogues[hit['corpus_id']], min(max(hit['score'], 0.001), 1)) for hit in hits if
                   self.dialogues[hit['corpus_id']] != dialogue]
           # hits_cp = [item for item in hits if item[1] >= positive_threshold]  # positive predictions
